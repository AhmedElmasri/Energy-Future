...
def show_model_results(df, model, scaler):
    ...
    if model_option == "Random Forest":
        ...
        ax.scatter(y_test, rf_pred, alpha=0.5, edgecolor='k', color='royalblue')
        ...
        fig, ax = plt.subplots(figsize=(8, 5))
        ax.scatter(y_test, lr_pred, alpha=0.5, edgecolor='gray', color='green')
        ax.plot([y.min(), y.max()], [y.min(), y.max()], 'r--')
        ax.set_xlabel("Actual Price")
        ax.set_ylabel("Predicted Price")
        ...
        fig, ax = plt.subplots(figsize=(8, 5))
        ax.scatter(y_test, rf_pred, alpha=0.4, label="Random Forest", color="royalblue", edgecolor='k')
        ax.scatter(y_test, lr_pred, alpha=0.4, label="Linear Regression", color="green", edgecolor='gray')
